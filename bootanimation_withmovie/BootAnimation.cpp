...
#define USER_BOOTVIDEO_FILE   "/system/media/boot.mp4"
#define USER_SHUTVIDEO_FILE   "/system/media/shut.mp4"

#define MUTE_VIDEO_SOUND    1
...
//	mSession->setOrientation(0, ISurfaceComposer::eOrientationDefault,0);
    ...
    mPlayer = NULL;
    mBootVideo = false;
    if(bSetBootOrShutDown && (access(USER_BOOTVIDEO_FILE, R_OK) == 0 )){
        mBootVideo = true;
    } else if (!bSetBootOrShutDown && (access(USER_SHUTVIDEO_FILE, R_OK) == 0 )){
        mBootVideo = true;
    }
...
BootAnimation::~BootAnimation() {
    ...
    if(mPlayer != NULL){
        XLOGD("[BootAnimation] distructor, stop player and clear surface.");
        mPlayer->stop();
        mPlayer = NULL;
...
bool BootAnimation::threadLoop()
{
    ...
    if(!mBootVideo){
        ...
        if (mPlaybackComplete && bBootOrShutDown && bootVidiocheckExit()){
             // No need to force exit anymore
...
bool BootAnimation::playBootVideo(){
    mAssets.addDefaultAssets();

    XLOGD("[BootAnimation] Enter playBootVideo");
    ...
    do {
        ...
        if (binder != 0)
            break;
        XLOGW("media player not published, waiting...");
        usleep(500000); // 0.5 s
    } while (true);
    ...
    status_t mediastatus = mPlayer->setDataSource(bBootOrShutDown?USER_BOOTVIDEO_FILE:USER_SHUTVIDEO_FILE, NULL);
    if (mediastatus == NO_ERROR) {
    #if MUTE_VIDEO_SOUND
        ...
        mPlayer->setVideoSurfaceTexture(s->getIGraphicBufferProducer());        
        mediastatus = mPlayer->prepare();
    }
    if (mediastatus == NO_ERROR) {
        mediastatus = mPlayer->start();
    ...
    return (mediastatus == NO_ERROR);
}

void BootAnimation::notify(int msg, int ext1, int ext2, const Parcel *obj)
{
	ALOGE("[BootAnimation], in notify, msg = %d", msg);
...
bool BootAnimation::bootVidiocheckExit() { //Jelly Bean added
    // Allow surface flinger to gracefully request shutdown
    char value[PROPERTY_VALUE_MAX];
    property_get(EXIT_PROP_NAME, value, "0");
    int exitnow = atoi(value);
...
void BootAnimation::getResourceFile(int index, void * path) {
    ...
    case INDEX_BOOT_ANIMATION:
		if(is_charging_boot()){
	        ...
	        strncpy(pathAddr, "/system/media/", strlen("/system/media/"));
	        strncat(pathAddr, mResourceFiles[5], strlen(
	                mResourceFiles[5]));
			...
			memset(pathAddr, 0, strlen(pathAddr));
	        strncpy(pathAddr, "/system/media/", strlen("/system/media/"));
	        strncat(pathAddr, mResourceFiles[0], strlen(
	                mResourceFiles[0]));
	        if ((access(pathAddr, R_OK) == 0) && (mZip.open(pathAddr)
	                == NO_ERROR)) {
	            mAndroidAnimation = false;
